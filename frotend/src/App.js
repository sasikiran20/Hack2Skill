...
function App() {
  ...
  const [stripeApiKeyError, setStripeApiKeyError] = useState(null);
    ...
    const getStripeApiKey = async () => {
      try {
        ...
        if (data.stripeApiKey) {
          ...
          setStripeApiKeyError(null);
        } else {
          setStripeApiKeyError('Stripe API key is missing or invalid');
        }
      } catch (error) {
        console.error('Error fetching Stripe API key:', error);
        setStripeApiKeyError(error.message);
      }
    };
      ...
      <Router>
        ...
        {stripeApiKeyError && (
          <div className="error-message">
            Error loading Stripe: {stripeApiKeyError}
          </div>
        )}
